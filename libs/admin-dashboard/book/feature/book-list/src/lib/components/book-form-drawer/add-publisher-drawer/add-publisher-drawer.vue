...
function addPublisher(event: FormSubmitEvent) {
  if (!event.valid) return;

  const { name } = event.values;

  store.addPublisher({ name }).then(() => {
...
<template>
  ...
  <Button class="h-fit mt-7" aria-label="Add publisher" v-tooltip="'Add publisher'" icon="pi pi-plus"
    @click="formVisibility = true" severity="secondary" />

  <Drawer header="Add publisher" v-model:visible="formVisibility" class="max-w-[40rem] w-full rounded-r-base">
    <Form @submit="addPublisher" :resolver="publisherResolver"
      ...
      <Button :loading="store.addLoading" type="submit">Add publisher</Button>
    </Form>
  </Drawer>
</template>
